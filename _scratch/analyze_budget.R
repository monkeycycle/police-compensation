wps_tax_supported_expenses <- wps_annual_reports %>%
  ...
  wps_tax_supported_expenses_v_inflation <- wps_annual_reports %>%
    select(year_date, tax_supported_expenses) %>%
    mutate(
...
wps_per_capita_cost <- wps_annual_reports %>%
...
# wps_tax_supported_expenses_5year =
